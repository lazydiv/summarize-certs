...
export const UpdateCertModal = () => {
    const { isOpen, type, onClose, data } = useModal();
    const router = useRouter();
    const { Certificate } = data;
    const isModelOpen = isOpen && type === 'updateCert';
    ...
    useEffect(() => {
        if (Certificate) {
            form.setValue('name', Certificate.name);
            form.setValue('course', Certificate.course);
            form.setValue('title', Certificate.title);
            form.setValue('status', Certificate.status);
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            ...
            await axios.patch(`/api/cert`, {data: values , id: Certificate?.id});
            form.reset();
            router.refresh()
            onClose();
        } catch (error) {
